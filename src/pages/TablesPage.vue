...
const unassignedList = computed<Guest[]>({
  ...
  // Called after any drop touching this panel; unassigning is idempotent,
  // and guests dragged out are no longer in `list`.
  set: (list) => {
    for (const guest of list) store.unassignGuest(guest.id)
  },
})
      ...
      <aside class="lg:w-64 lg:shrink-0">
        <div class="rounded-xl border border-stone-200 bg-white p-3">
          ...
          <p class="mt-2 text-xs text-stone-400">
            Drag guests onto a table, or click a guest then a table.
          </p>
        </div>
      </aside>
